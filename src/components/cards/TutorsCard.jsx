          ...
          .map((course) => (
            <Link key={course.id} href={`/courses/${course.id}`}>
              <div className="shadow-2xl shadow-[#1733792a] bg-white w-full p-4 flex flex-col gap-2 rounded-[20px] border border-[#37494e8b]">
                ...
                <p className="font-bold flex mt-11">
                  {" "}
                  <AiTwotoneFilePdf size={20} />
                  Download: <AiOutlineDownload size={20} /> {course.pdf}
                </p>
              </div>
            </Link>
          ))}
